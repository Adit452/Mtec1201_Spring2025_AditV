...
function setup(){
  ...
  restartSketch();
  setSeasonColors()
//making cloud x and y position so it can be used for the rain array
...
// array for stars
  for (let i = 0; i < 100; i++) {
    stars[i] =({ x: random(width), y: random(300), size: random(1, 3) });
  }
...
if (season === 3) { // Winterss
  for (let i = 0; i < 100; i++) { // 100 Snowflakes
    let snowX = random(150,1600);
    let snowY = random(height)
    let size = random(3, 8);
    fill(255);
    ellipse(snowX, snowY, size, size);
  }
}
//array for rain taken from this youtube video: https://www.youtube.com/watch?v=LUfivoidnLw
if (season === 0 || season === 2) {
  fill(0, 200, 255);
  noStroke();
